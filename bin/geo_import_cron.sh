...
pushd $supportingFilesPath
    ...
    bsub -q production-rh74 -cwd `pwd` -M 80000 -R "rusage[mem=80000]" -o ${bulkORsinglecell}_geo_import_$today.out -e ${bulkORsinglecell}_geo_import_$today.err "$projectRoot/bin/import_geo_subs.pl -n -x -f latest_${bulkORsinglecell}_geo_accessions.txt -o $outputPath"
    if [ $? -ne 0 ]; then
