...
pushd $supportingFilesPath
    ...
    bsub -q production-rh7 -cwd `pwd` -M 80000 -R "rusage[mem=80000]" -o ${bulkORsinglecell}_geo_import_$today.out -e ${bulkORsinglecell}_geo_import_$today.err "$projectRoot/geo_import/bin/import_geo_subs.pl -x -f latest_${bulkORsinglecell}_geo_accessions -o $outputPath"
    if [ $? -ne 0 ]; then
