...
def fetch_gse_ids(sraid):
    ...
    search_params = {'db': 'gds', 'term': sra, 'retmode': 'json'}
...
def write_dataframe_to_tsv(filename, dataframe, output):
    file_path = os.path.join(output, filename)
    file_name = file_path + '.tsv'
    dataframe.to_csv(file_name, sep='\t', index=False, header=None)
...
def get_args():
    parser = argparse.ArgumentParser(
        description='This program retrieves ENA-ids and organism list for bulk or singlecell RNA-seq experiments, '
                    'and converts it to GEO ids. Filtering ids the ones which already exist in ArrayExpress')
...
if __name__ == "__main__":
    ...
    write_dataframe_to_tsv(filename='geo_' + args['type'] + '_rnaseq', dataframe=gse, output=args['output'])
